...
class Style:
  ...
  def damageEquipment(self, attacker, attack_handler):
    for data in self.equipmentUsed:
      attack_handler.consumeEquipment(attacker, data[0], data[1]);
  ...
  def attack(self, attacker, defender, game, combat_handler):
    ...
    self.damageEquipment(attacker, attack_handler);
