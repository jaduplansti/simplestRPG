...
class DamageHandler:
  ...
  def reduceDamageByDefense(self, dmg, attacker, defender):
    ...
    if event == "defense_ignored":
      self.ui.animatedPrint("damage handler", "defense ignored", [attacker.name, defender.name]);
      return dmg;
    elif dmg - defender.stats["defense"] <= 0:
      self.ui.animatedPrint("damage handler", "no damage", [attacker.name, defender.name]);
      return 0;
    return dmg - defender.stats["defense"];
    
  def calculateDamage(self, name, attacker, defender):
    if name == "punch":
      dmg = self.reduceDamageByDefense(attacker.stats["strength"], attacker, defender);
      return max(dmg, 0);
      
    elif name == "strong punch":
      dmg = self.reduceDamageByDefense(attacker.stats["strength"] * 1.3, attacker, defender);
      return max(dmg, 0);
  ...
  def handleAttack(self, attacker, defender):
    if attacker.attack_style == "basic":
      ...
      if preset is 1:
        ...
        self.ui.animatedPrint("basic style", "normal punch", [attacker.name, defender.name, dmg]);
        attacker.attackEnemy(dmg);
      elif preset is 2:
        dmg = self.damage_handler.calculateDamage("strong punch", attacker, defender);
        self.ui.animatedPrint("basic style", "strong punch", [attacker.name, defender.name, dmg]);
        attacker.attackEnemy(dmg);
