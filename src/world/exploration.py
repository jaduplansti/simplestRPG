...
class Exploration:
  ...
  def handleGetNext(self, area):
    while True:
      self.ui.clear();
      try:
        for loc in area["next"]: self.ui.normalPrint(f"- [yellow underline]{loc}[reset] ({AREAS[loc]['position']} pos) ⬆\n");
        for loc in area["prev"]: self.ui.normalPrint(f"- [yellow underline]{loc}[reset] ({AREAS[loc]['position']} pos) ⬇\n");
  ...
  def explore(self):
    ...
    destination_data = self.handleGetNext(area);
    if destination_data == -1: return;
    destination, destination_name = destination_data;
