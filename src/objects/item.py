...
class Item:
  ...
  def typeToVal(self):
    ...
    elif self._type in ["food", "ingredient"]: return 6;
...
def use_potion(item, game, combat_handler, user):
  ...
  if item.name == "health potion":
    ...
    strength_increased = round((user.stats["strength"] * 0.2) / user.stats["luck"]);
...
def use_book(item, game, combat_handler, user):
  if item.name == "bible":
    ...
    for skill in all_skills: 
      if all_skills[skill]["skill"]._style == None: skills.append(skill);
...
ITEMS = {
  ...
  "wheat": {
    "item": Item(name="wheat", rank="F", rarity="common", weight=0.08, _type="ingredient", desc="Wheat grass."),
    "action": None,
  },
}
